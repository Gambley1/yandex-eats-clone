...
Future<Response> onRequest(RequestContext context) async {
  final handler = webSocketHandler(
    (channel, protocol) async {
      ...
      await db.execute('LISTEN user_order_status_changed');
      ...
      db.channels.all.listen(
        (notification) {
          ...
          if (connectionChannel == ConnectionChannel.userOrderStatusChanged) {
            final payload = notification.payload;
            final order = jsonDecode(payload) as Map<String, dynamic>;

            final orderId = order['id'] as String;
            final status = order['status'] as String;
            final userUid = order['user_uid'] as String;

            channel.sink.add(jsonEncode([status, orderId, userUid]));
          }
        },
        onDone: close,
        onError: (_) => close(),
